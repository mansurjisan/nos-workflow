...
if [ -z "${DATA_prep_gfs}" ]; then
    echo "FATAL ERROR: DATA_prep_gfs is not set. Exiting to prevent accidental file deletion."
    echo "Please set DATA_prep_gfs to a valid working directory path before running this script."
    exit 1
fi

# Additional safety: ensure path is not root or system directory
case "${DATA_prep_gfs}" in
    /|/bin|/boot|/dev|/etc|/home|/lib*|/opt|/proc|/root|/run|/sbin|/sys|/tmp|/usr|/var|/mnt|/media)
        echo "FATAL ERROR: DATA_prep_gfs='${DATA_prep_gfs}' appears to be a system directory."
