...
if [ -n "${OFS_CONFIG}" ] && [ -f "${OFS_CONFIG}" ]; then
    ...
    if [ -f "${_yaml_to_env}" ]; then
        echo "Loading OBC nudge config from YAML: ${OFS_CONFIG}"
...
if [ "${ALLOW_CONDA_ACTIVATE:-NO}" = "YES" ] && { [ -n "$CONDA_PREFIX" ] || [ -f "${HOME}/miniconda3/etc/profile.d/conda.sh" ]; }; then
    ...
    echo "Conda environment nos_ofs_prep activated"
fi

# ---------------------------> SAFETY CHECK: Validate required environment variables
# This prevents catastrophic deletion if variables are not set
if [ -z "${DATA_prep_rtofs}" ]; then
    echo "FATAL ERROR: DATA_prep_rtofs is not set. Exiting to prevent accidental file deletion."
    echo "Please set DATA_prep_rtofs to a valid working directory path before running this script."
    exit 1
fi

# Additional safety: ensure path is not root or system directory
case "${DATA_prep_rtofs}" in
    /|/bin|/boot|/dev|/etc|/home|/lib*|/opt|/proc|/root|/run|/sbin|/sys|/tmp|/usr|/var|/mnt|/media)
        echo "FATAL ERROR: DATA_prep_rtofs='${DATA_prep_rtofs}' appears to be a system directory."
...
# Note: nudge script uses slightly different 3D ROI than OBC 3D TH script
idx_x1_2ds=${idx_x1_2ds:-2805}
idx_x2_2ds=${idx_x2_2ds:-2923}
idx_y1_2ds=${idx_y1_2ds:-1598}
idx_y2_2ds=${idx_y2_2ds:-2325}

idx_x1_3dz=${idx_x1_3dz_nudge:-422}
idx_x2_3dz=${idx_x2_3dz_nudge:-600}
idx_y1_3dz=${idx_y1_3dz_nudge:-94}
idx_y2_3dz=${idx_y2_3dz_nudge:-835}

echo "RTOFS ROI 2D: x1=$idx_x1_2ds x2=$idx_x2_2ds y1=$idx_y1_2ds y2=$idx_y2_2ds"
echo "RTOFS ROI 3D (nudge): x1=$idx_x1_3dz x2=$idx_x2_3dz y1=$idx_y1_3dz y2=$idx_y2_3dz"

# v6 RTOFS-2D, nudge dmn: 2805  2923  1598  2325
# v6 RTOFS-3D, nudge dmn: [422  600   94  835]
