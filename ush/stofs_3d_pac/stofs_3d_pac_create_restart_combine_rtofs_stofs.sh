...
#####################################################################################
#  Name: stofs_3d_pac_create_restart_combine_rtofs_stofs.sh                         #
...
# set +H
 ...
 fn_this_script=stofs_3d_pac_create_restart_from_rtofs.sh
...
# ---------------------------> Global Variables
  fn_nco_ssh=${FIXstofs3d}/stofs_3d_pac_obc_3dth_cvt_ssh.nco
  fn_nco_tsuv=${FIXstofs3d}/stofs_3d_pac_obc_3dth_cvt_tsuv.nco  

  fn_exe_gen_hotstart=${EXECstofs3d}/stofs_3d_pac_gen_hot_from_hycom
  fn_input_gen_hotstart=${FIXstofs3d}/stofs_3d_pac_hotstart_nc.in
...
# --------------------------> create list of 3d daily RTOFS files
    hr_rst=0${yyyymmddhr_rst:8:2}
    fn_rtofs_3ds=rtofs_glo_3dz_n024_daily_3zsio.nc
    fn_rtofs_3dt=rtofs_glo_3dz_n024_daily_3ztio.nc
    fn_rtofs_3du=rtofs_glo_3dz_n024_daily_3zuio.nc
    fn_rtofs_3dv=rtofs_glo_3dz_n024_daily_3zvio.nc
   ...
   for k in ${days[@]}; do
            kk=$((k+1))
      date_k=`date -d "${yyyymmdd_rst} ${kk} days ago" +%Y%m%d`
      fn_3d_i=${COMINrtofs}/rtofs.${date_k}/${fn_rtofs_3ds}

  yyyymmdd_today='20230606'
  yyyymmdd_prev='20230605'

                fn_comb=rtofs_glo_3dz_n024_daily_trim_comb0${k}.nc
        in02='t';in03='temperature';list_var_oi="MT,Date,Longitude,Latitude,${in03}"
###  list_var_oi='MT,Date,Longitude,Latitude,temperature,salinity,u,v'
###      fn_in="${COMINrtofs}/rtofs.${date_k}/${fn_rtofs_3d${in02}}"
      fn_in="${COMINrtofs}/rtofs.${date_k}/${fn_rtofs_3dt}"
      fn_out="fn_out.nc"
echo $fn_in; echo $list_var_oi
    ncks -O -d X,$idx_x1_3dz,$idx_x2_3dz,2 -d Y,$idx_y1_3dz,$idx_y2_3dz,2 -v $list_var_oi  $fn_in  $fn_out
cp -rp $fn_out $fn_comb   

        in02='s';in03='salinity';list_var_oi="MT,Date,Longitude,Latitude,${in03}"
      fn_in="${COMINrtofs}/rtofs.${date_k}/${fn_rtofs_3ds}"
      fn_out="fn_out.nc"
echo $fn_in; echo $list_var_oi
    ncks -O -d X,$idx_x1_3dz,$idx_x2_3dz,2 -d Y,$idx_y1_3dz,$idx_y2_3dz,2 -v $list_var_oi  $fn_in  $fn_out
ncks -A -v $in03 $fn_out $fn_comb

        in02='u';in03='u';list_var_oi="MT,Date,Longitude,Latitude,${in03}"
      fn_in="${COMINrtofs}/rtofs.${date_k}/${fn_rtofs_3du}"
      fn_out="fn_out.nc"
echo $fn_in; echo $list_var_oi
    ncks -O -d X,$idx_x1_3dz,$idx_x2_3dz,2 -d Y,$idx_y1_3dz,$idx_y2_3dz,2 -v $list_var_oi  $fn_in  $fn_out
ncks -A -v $in03 $fn_out $fn_comb

        in02='v';in03='v';list_var_oi="MT,Date,Longitude,Latitude,${in03}"
      fn_in="${COMINrtofs}/rtofs.${date_k}/${fn_rtofs_3dv}"
      fn_out="fn_out.nc"
echo $fn_in; echo $list_var_oi
    ncks -O -d X,$idx_x1_3dz,$idx_x2_3dz,2 -d Y,$idx_y1_3dz,$idx_y2_3dz,2 -v $list_var_oi  $fn_in  $fn_out
ncks -A -v $in03 $fn_out $fn_comb

           LIST_fn_3d_comb+="${fn_comb} "
   done

# --------------------------> create list of average 3d daily RTOFS files
###  list_var_oi='MT,Date,Longitude,Latitude,temperature,salinity,u,v'
    days=(0 1 2)
    LIST_fn_final_3d=''
   ...
   for k in ${days[@]}; do
            kk=$((k+1))
      fn_in_1=rtofs_glo_3dz_n024_daily_trim_comb0${k}.nc
      fn_in_2=rtofs_glo_3dz_n024_daily_trim_comb0${kk}.nc
      fn_out_12z=rtofs_glo_3dz_n024_daily_trim_combavg0${k}.nc
      ncra -O ${fn_in_1} ${fn_in_2} ${fn_out_12z}

    # 3D: files: check file sizes
      FILESIZE=200000000
      fn_3d_k_sz=${fn_out_12z}
...
if [[ $cnt_2d -gt 1 ]] && [[ $cnt_3d -gt 1 ]]; then
   ...
   echo "RTOFS files being used before qc: "
   echo "LIST_fn_final_2d = ${LIST_fn_final_2d}"
   echo "LIST_fn_final_3d = ${LIST_fn_final_3d}"
   echo 
...
# --------------------------> Process rotfs data
  ...
  do
    ...
    ncks -O -d X,$idx_x1_2ds,$idx_x2_2ds,2 -d Y,$idx_y1_2ds,$idx_y2_2ds,2 -v $list_var_oi  $fn_in  $fn_out   
...
# --------------------------> create {elev2D.th.nc, SAL_3D.th.nc, TEM_3D.th.nc, uv3D.th.nc}
 ...
 ln -sf ${FIXstofs3d}/stofs_3d_pac_vgrid.in       vgrid.in
 ln -sf ${FIXstofs3d}/stofs_3d_pac_hgrid.ll       hgrid.ll
 ln -sf ${FIXstofs3d}/stofs_3d_pac_hgrid.gr3      hgrid.gr3
 ln -sf ${FIXstofs3d}/stofs_3d_pac_estuary.gr3    estuary.gr3

 ln -sf ${FIXstofs3d}/stofs_3d_pac_ocean.dbf      ocean.dbf
 ln -sf ${FIXstofs3d}/stofs_3d_pac_ocean.prj      ocean.prj
 ln -sf ${FIXstofs3d}/stofs_3d_pac_ocean.shp      ocean.shp 
 ln -sf ${FIXstofs3d}/stofs_3d_pac_ocean.shx      ocean.shx 
...
###  if [[ $(find ${fn_restart_rtofs_ftn} -type f -size  +20G 2>/dev/null) ]]; then
  if [[ $(find ${fn_restart_rtofs_ftn} -type f -size  +15G 2>/dev/null) ]]; then
...
###        if [[ $(find ${fn_hotstart_oper_chk} -type f -size  +20G 2>/dev/null) ]];
        if [[ $(find ${fn_hotstart_oper_chk} -type f -size  +15G 2>/dev/null) ]];
        then
...
###  if [[ $(find $fn_hotstart_cbn -type f -size  +20G 2>/dev/null) ]]; then
  if [[ $(find $fn_hotstart_cbn -type f -size  +15G 2>/dev/null) ]]; then
  ...
  if [[ $flag_rst_cbn_success -eq 1 ]]; then
     ...
     cp -pfL ${fn_restart_rtofs_std} ${fn_restart_rerun}_rtofs_only
