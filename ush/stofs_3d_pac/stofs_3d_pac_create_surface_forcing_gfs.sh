...
############################################################################
#  Name: stofs_3d_pac_create_surface_forcing_gfs.sh                        #
#  This script read the NCEP/GFS data to create the GFS based surface      #
#  forcing files, stofs_3d_pac.t12z.gfs.{air,prc,rad}.nc for the nowcast   #
...
echo 'stofs_3d_pac_create_surface_forcing_gfs.sh started at UTC' `date -u +%Y%m%d%H`
...
# ---------------------------> Global Variables
  fn_nco_update_time_varName=${FIXstofs3d}/stofs_3d_pac_gfs_input_nco_update_var.nco
  fn_txt_sflux_inputs=${FIXstofs3d}/stofs_3d_pac_sflux_inputs.txt
...
### not sure about the backup 1 or non-backup, and fn_ori and fn_std
   if [[ ${dim_k} -ge ${N_dim_cr_max} ]]; then
      ln -sf ${fn_ori} ${fn_std} 
      ln -sf ${fn_ori} ${fn_gfs_rad_schism}
      ln -sf ${fn_ori} ${fn_gfs_prc_schism}
      ln -sf ${fn_ori} ${fn_gfs_air_schism}
      ...
      ln -sf ${fn_std} ${fn_gfs_rad_schism}
      ln -sf ${fn_std} ${fn_gfs_prc_schism}
      ln -sf ${fn_std} ${fn_gfs_air_schism}
...
echo
echo "The script stofs_3d_pac_create_surface_forcing_gfs.sh completed at date/time: " `date`
echo 
