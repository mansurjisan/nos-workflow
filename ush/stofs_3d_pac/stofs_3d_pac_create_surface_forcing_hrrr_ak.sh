...
# ------> form the input file lists 
  # nowcast, yesterday: t09z, f03
  list_num=`seq -f "%02g" 3 1 3`
  
  str_base_prev=${COMINhrrr}/hrrr.${yyyymmdd_prev}/alaska
  list_fn_nowcast=''
  for num_k in $list_num
  do
    fn_k=${str_base_prev}/hrrr.t09z.wrfsfcf${num_k}.ak.grib2
    list_fn_nowcast=${list_fn_nowcast}' '${fn_k}
  done

  # nowcast, yesterday: t12z, f01-f03
  list_num=`seq -f "%02g" 1 1 3`
  
  str_base_prev=${COMINhrrr}/hrrr.${yyyymmdd_prev}/alaska
  for num_k in $list_num
  do
    fn_k=${str_base_prev}/hrrr.t12z.wrfsfcf${num_k}.ak.grib2
    list_fn_nowcast=${list_fn_nowcast}' '${fn_k}
  done

  # nowcast, yesterday: t15z, f01-f03
  list_num=`seq -f "%02g" 1 1 3`
  
  str_base_prev=${COMINhrrr}/hrrr.${yyyymmdd_prev}/alaska
  for num_k in $list_num
  do
    fn_k=${str_base_prev}/hrrr.t15z.wrfsfcf${num_k}.ak.grib2
    list_fn_nowcast=${list_fn_nowcast}' '${fn_k}
  done

  # nowcast, yesterday: t18z, f01-f03
  list_num=`seq -f "%02g" 1 1 3`
  
  str_base_prev=${COMINhrrr}/hrrr.${yyyymmdd_prev}/alaska
  for num_k in $list_num
  do
    fn_k=${str_base_prev}/hrrr.t18z.wrfsfcf${num_k}.ak.grib2
    list_fn_nowcast=${list_fn_nowcast}' '${fn_k}
  done

  # nowcast, yesterday: t21z, f01-f03
  list_num=`seq -f "%02g" 1 1 3`
  
  str_base_prev=${COMINhrrr}/hrrr.${yyyymmdd_prev}/alaska
  for num_k in $list_num
  do
    fn_k=${str_base_prev}/hrrr.t21z.wrfsfcf${num_k}.ak.grib2
    list_fn_nowcast=${list_fn_nowcast}' '${fn_k}
  done

  # nowcast, today: t00z, f01-f03
  list_num=`seq -f "%02g" 1 1 3`
  
  str_base_today=${COMINhrrr}/hrrr.${yyyymmdd_today}/alaska
  for num_k in $list_num
  do
    fn_k=${str_base_today}/hrrr.t00z.wrfsfcf${num_k}.ak.grib2
    list_fn_nowcast=${list_fn_nowcast}' '${fn_k}
  done

  # nowcast, today: t03z, f01-f03
  list_num=`seq -f "%02g" 1 1 3`
  
  str_base_today=${COMINhrrr}/hrrr.${yyyymmdd_today}/alaska
  for num_k in $list_num
  do
    fn_k=${str_base_today}/hrrr.t03z.wrfsfcf${num_k}.ak.grib2
    list_fn_nowcast=${list_fn_nowcast}' '${fn_k}
  done

  # nowcast, today: t06z, f01-f03
  list_num=`seq -f "%02g" 1 1 3`
  
  str_base_today=${COMINhrrr}/hrrr.${yyyymmdd_today}/alaska
  for num_k in $list_num
  do
    fn_k=${str_base_today}/hrrr.t06z.wrfsfcf${num_k}.ak.grib2
    list_fn_nowcast=${list_fn_nowcast}' '${fn_k}
  done

  # nowcast, today: t09z, f01-f03
  list_num=`seq -f "%02g" 1 1 3`
  
  str_base_today=${COMINhrrr}/hrrr.${yyyymmdd_today}/alaska
  for num_k in $list_num
  do
    fn_k=${str_base_today}/hrrr.t09z.wrfsfcf${num_k}.ak.grib2
    list_fn_nowcast=${list_fn_nowcast}' '${fn_k}
  done

  # firecast, today: t12, f01-48
  list_num=`seq -f "%02g" 1 1 48`
  
  str_base_today=${COMINhrrr}/hrrr.${yyyymmdd_today}/alaska
  list_fn_forecast=''
  for num_k in $list_num
  do
    fn_k=${str_base_today}/hrrr.t12z.wrfsfcf${num_k}.ak.grib2
    list_fn_forecast=${list_fn_forecast}' '${fn_k}
  done

# concatenate dir/file names
 LIST_fn_all="${list_fn_nowcast} "
 LIST_fn_all+="${list_fn_forecast[@]} "
...
list_var_oi='TMP:2 m above|RH:2 m above|SPFH:2 m above|MSLMA:mean sea level|PRATE|UGRD:10 m above|VGRD:10 m above|ALBDO:surface|DSWRF:surface|USWRF:surface|DLWRF:surface|ULWRF:surface'
...
###      export err=$?;  #err_chk
      ...
      fn_roi=HRRR_voi_rio_${str_xxx_cnt}.grb2
      $WGRIB2  -s  $fn_hrrr_k  | egrep "$list_var_oi" | $WGRIB2  -i  $fn_hrrr_k  -grib  $fn_roi  >> $pgmout 2> errfile 
      export err=$?;  #err_chk
  ...
  ncap2 -O -s 'where(lon<0) lon=lon+360' $fn_merged_sflux $fn_merged_sflux
  export err=$?;  #err_chk
