...
def test_ww3_pbs_scripts_sized_for_3600_ranks():
    """nowcast/forecast select= lines are resized to 30 nodes @ 120 ppn
    (3600 ranks = datm 120 + schism 2794 + wav 686), matching
    ...
    for stage in ("NOWCAST", "FORECAST"):
        ...
        if not pbs.is_file():
            pytest.skip(f"{pbs.name} not present")
        text = pbs.read_text()
        assert "select=30:ncpus=128:mpiprocs=120:ompthreads=1" in text
        assert re.search(r"TOTAL_TASKS:-3600\}", text)
