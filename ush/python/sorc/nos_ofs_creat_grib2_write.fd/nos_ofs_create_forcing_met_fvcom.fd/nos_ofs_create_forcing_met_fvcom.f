...
!       IF(trim(OCEAN_MODEL) .EQ. 'SELFE')IHHE=IHHE+1
       ...
       IF(trim(OCEAN_MODEL).EQ.'SCHISM') IHHE=IHHE+3   !!  machuan
...
!     1    BUFFER(IND_HOUR:IND_HOUR_END)
 ...
 !            IF (TRIM(OCEAN_MODEL) .EQ. "SELFE" )THEN  ! air temperature in SELFE is in Kelvin 
             IF (TRIM(OCEAN_MODEL).EQ."SELFE".or.TRIM(OCEAN_MODEL).EQ.
     &       "SCHISM" )THEN  ! air temperature in SELFE is in Kelvin
...
C                 FFJD=LAT_XX_P - latsub(I,J)
	...
	ENDIF  
     ...
     1		for current FVCOM'
	        ...
	        STOP
!              ELSEIF (trim(OCEAN_MODEL) .EQ. 'SELFE')THEN
              ELSEIF (trim(OCEAN_MODEL).EQ.'SELFE'.or.trim(OCEAN_MODEL)
     &        .EQ.'SCHISM')THEN
...
!                ELSEIF (TRIM(OCEAN_MODEL) .EQ. "SELFE" )THEN
     ...
     &          .EQ.'SCHISM')THEN
                ...
                WRITE(*,*)'avg_tdair=',avg_tdair,'avg_tcdc=',avg_tcdc,'avg_evp=',avg_evp,'avg_prate=',avg_prate,'avg_spfh=',avg_spfh
                DO ITMP=1,NODEm
                   ...
                   if (spfhm(ITMP) .LE. 0.0) spfhm(ITMP)=0.001
                   if (spfhm(ITMP) .GT. 0.02)spfhm(ITMP)=0.02
                ENDDO   
!        IF (IGRD .GT. 0)THEN
