...
C
          ...
          DO
            ...
            IF (LP .EQ. LPB) exit
            KN = ABS(LIST(LP))
            IF (KN .LT. IFRST  .OR.  KN .GT. ILAST) THEN 
              IER = 5
              RETURN
            ENDIF
          ENDDO
...
C
     ...
     .    .OR.  NCC .LT. 0  .OR.  N .LT. 3) THEN 
        IER = -1
        RETURN
      ENDIF
      LCCIP1 = N+1
      DO 1 I = NCC,1,-1
        IF (LCCIP1-LCC(I) .LT. 3) THEN  
          IER=-1
          RETURN
        ENDIF
        LCCIP1 = LCC(I)
    1   CONTINUE
      IF (KK .GT. LCCIP1) THEN 
        IER = -1
        RETURN
      ENDIF
...
C
      ...
      IF (I3 .NE. 0) THEN
        L = I1
        IF (XK .EQ. X(L)  .AND.  YK .EQ. Y(L)) THEN  
          IER = 1
          RETURN
        ENDIF
        L = I2
 ...
 5    CONTINUE
      DO
       LP = LSTPTR(LEND(IO1),IO2,LIST,LPTR)
        IF (LIST(LP) .LT. 0) exit
        LP = LPTR(LP)
        IN1 = ABS(LIST(LP))
        IF ( CRTRI(NCC,LCC,IO1,IO2,IN1) ) exit
C
C Swap test:  if a swap occurs, two new arcs are
C             opposite K and must be tested.
C
       IF ( .NOT. SWPTST(IN1,KK,IO1,IO2,X,Y) ) exit
...
!    7
      DO
        IF (LIST(LP) .EQ. NR) EXIT
        LP = LPTR(LP)
        IF (LP .NE. LPL) CYCLE
        IF (LIST(LP) .NE. NR) THEN
          IER = 3
          WRITE (*,130) IN1, IN2
         RETURN
       ENDIF 
...
!    6
          DO
            LPK = LPTR(LPK)
            NC = ABS(LIST(LPK))
            IF (NC .NE. NKFOR  .AND.  NC .NE. NKBAK) CYCLE
            EXIT
...
C     N1 from I1 to I2.
        ...
        DO
          ...
          LISTP(L) = N1
          LPTRP(L) = LP1
          IF (N1 .NE. I2  .AND.  LP1 .LT. LMAX) CYCLE
          EXIT
        ENDDO
        L = LP1
        LISTP(L) = 0
        LPTRP(L) = 1
      ENDIF
CC Initialize variables for a loop on arcs N1-N2 opposite P
...
C
      NNA = NA
      MAXIT = NIT
      IF (NNA .LT. 0  .OR.  MAXIT .LT. 1) THEN
        NIT = 0
        IER = 2
        RETURN
      ENDIF
...
C
      IF (COS1 .GE. 0.  .AND.  COS2 .GE. 0.) THEN  
        SWPTST = .FALSE.
        RETURN
      ENDIF
      IF (COS1 .LT. 0.  .AND.  COS2 .LT. 0.) THEN  
        SWPTST = .TRUE.
        RETURN
      ENDIF
...
C
            IF (ABS(LIST(LP)) .NE. I2) THEN 
                ...
                WRITE(*,*)'I1 is a neighbor of I2, but I2 not a
     .                     neighbor of I1'
                RETURN
            ENDIF
            KN = 0
...
C
          DO
            LP = LPTR(LP)
            N1 = ABS(LIST(LP))
            IF (N1 .NE. N0FOR  .AND.  N1 .NE. N0BAK) CYCLE
            EXIT
