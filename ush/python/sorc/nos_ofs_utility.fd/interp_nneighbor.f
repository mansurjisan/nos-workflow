...
c                              
     ...
     & NINP,XOUT,YOUT,AOUT)
      ...
      parameter		(np_max=200000,nd_max=2,nt_max=2*np_max)
      parameter		(nh_max=200000,nnpn_max =50,nmax=3*nt_max+np_max)
      parameter		(nv_max=200000)
c
...
c					Set debug mode for nn routines
      ...
      clockwise=.false.
      nnext=.true.
      write(*,*)'test in INTERP_NNEIGHBORS'
...
!      ymax=jm
      favg=0
      do i=1,NDATA
       points(1,i)=dble(XINP(i)*100.0)
       points(2,i)=dble (YINP(i)*100.0)
       fvals(i)=dble(AINP(i))
       favg=favg+fvals(i)
      enddo
      favg=favg/NDATA
...
c					Perform natural neighbour interpolation
      do i=1,NINP
        write(*,*)'test before calling nn2d i= ',i
          xd=XOUT(I)
          yd=YOUT(I)
          AOUT(I)=0.
...
c                                       read in Delaunay vertices
              ...
              read(dmode,*)
              loopread: do
              read(dmode,*,end=3,err=2)
cilya  1           read(dmode,*,end=3,err=2)
...
cilya 10	call stackpairempty(k)
c					if stack empty then finish
        if(k.eq.1) exit loopstack1
cilya	if(k.eq.1)go to 100
...
cilya           do 121 ii=1,2
	      node = pairs(ii,i)
              do 122 kk=1,j
                 if(node.eq.nodes(kk)) cycle loop121a
cilya                if(node.eq.nodes(kk))go to 121
 122          continue
              j = j + 1
	      nodes(j) = node
              p(1,1) = points(1,node)
...
cilya           do 121 ii=1,2
	      node = pairs(ii,i)
              do 122 kk=1,j
                 if(node.eq.nodes(kk)) cycle loop121b
cilya                if(node.eq.nodes(kk))go to 121
 122          continue
              j = j + 1
	      nodes(j) = node
...
cilya           do 130 in = 1,3
              ...
              if(lnode(node)) cycle loop130a
cilya              if(lnode(node))go to 130
              lnode(node) = .true.
              nverts=0
...
cilya     i=0 not needed because i=0 after loop ends normally
          indx(i+1)=indxt
cilya          do 12 i=j-1,1,-1
cilya            if(arr(indx(i)).le.a)goto 2
cilya            indx(i+1)=indx(i)
cilya12        continue
cilya          i=0
cilya2         indx(i+1)=indxt
13      continue
...
cilya 3       continue
cilya          i=i+1
cilya        if(arr(indx(i)).lt.a)goto 3
        do 
          j=j-1
...
cilya 4       continue
cilya           j=j-1
cilya         if(arr(indx(j)).gt.a)goto 4
        if(j.ge.i)then
          itemp=indx(i)
          indx(i)=indx(j)
          indx(j)=itemp
        ...
        indx(l)=indx(j)
cilya         if(j.lt.i)goto 5
cilya         itemp=indx(i)
cilya         indx(i)=indx(j)
cilya         indx(j)=itemp
cilya        goto 3
cilya 5       indx(l)=indx(j)
        indx(j)=indxt
        jstack=jstack+2
...
c                                       to form the first triangle 
           v(1,1) = i1
           v(2,1) = i2
           colinear: do j=nodestart,num
cilya           do 10 j=nodestart,num
              i = j
              if(mode.eq.2)then
                  i = subset(j)
...
cilya                  go to 10
     ...
     &              *(points(2,i2)-points(2,i))
              del = del1-del2
              if(dabs(del).gt.eps) exit colinear
cilya              if(dabs(del).gt.eps) goto 11111
           end do colinear
           if (j.eq.num+1) stop 'all input data are in a line...'
           v(3,1) = istart
cilya           stop 'all input data are in a line...'
cilya 10        continue
cilya 11111      v(3,1) = istart
...
c					Delaunay triangulation
 	...
 	incrupdate: do j=nodestart,num
cilya 	do 100 j=nodestart,num


           p = j
           if(mode.eq.1.or.mode.eq.3)then
             if (inactive(j)) cycle incrupdate
cilya            if (inactive(j)) goto 100
           else if(mode.eq.2)then
	     p = subset(j)
           end if
             
           if(p.eq.istart) cycle incrupdate
cilya           if(p.eq.istart)go to 100
...
cilya              go to 10
	   ...
	   jnode = vertices(pos,t1)
    	   tnew = neighbour(c2(pos),t1)
	   if(tnew.ne.0) exit loop6
cilya	   if(tnew.eq.0) go to 6
           end do loop6
           t1 = tnew
	   cycle loop5
cilya	   go to 5 
...
cilya              go to 20
	   ...
	   jnode = vertices(pos,t2)
    	   pos = c2(pos)
    	   tnew = neighbour(c2(pos),t2)
	   if(tnew.ne.0) exit loop16
cilya	   if(tnew.eq.0)go to 16
           end do loop16
	   t2 = tnew
           cycle loop15
cilya	   go to 15
...
c					record triangle attached to p
	...
	call stackempty(k)
        if(k.eq.1) exit loopstackempty
cilya	if(k.ne.1)go to 50
        end do loopstackempty
        call stackflush()
...
c					loop while stack is not empty
        ...
        notzero: if(a.ne.0.or.b.ne.0.or.c.ne.0) then
cilya        if(a.eq.0.and.b.eq.0.and.c.eq.0)go to 100
...
c					triangles a and c
	...
	call stackempty(k)
        if (k.eq.1) exit loopinsertpoint
        end do loopinsertpoint
cilya	if(k.ne.1)go to 50
        end if notzero
cilya 100    continue
        call stackflush()
...
cilya           do 20 j=1,3
              ...
              i1 = vertices(j1,i)
              i2 = vertices(j2,i)
              if(k.eq.0) cycle test2Dj
cilya              if(k.eq.0)go to 20
              do 30 m=1,3
                 m1 = mod(m,3)+1
                 m2 = mod(m1,3)+1
                 k1 = vertices(m1,k)
     ...
     &                 k1.eq.i2.and.k2.eq.i1) cycle test2Dj
cilya                  if(neighbour(m,k).eq.i)then
cilya                    if(k1.eq.i1.and.k2.eq.i2.or.
cilya     &                 k1.eq.i2.and.k2.eq.i1)go to 20
                 else if(m.eq.3)then
...
cilya           do 120 j=1,4
              ...
              i2 = vertices(j2,i)
              i3 = vertices(j3,i)
              if(k.eq.0)cycle test3Dj
cilya              if(k.eq.0)go to 120
              do 130 m=1,4
                 m1 = mod(m,4)+1
                 m2 = mod(m1,4)+1
                 m3 = mod(m2,4)+1
     ...
     &                 (k1.eq.i3.and.k2.eq.i2.and.k3.eq.i1)) 
     &                 cycle test3Dj
cilya                     if((k1.eq.i1.and.k2.eq.i2.and.k3.eq.i3).or.
cilya     &                 (k1.eq.i1.and.k2.eq.i3.and.k3.eq.i2).or.
cilya     &                 (k1.eq.i2.and.k2.eq.i1.and.k3.eq.i3).or.
cilya     &                 (k1.eq.i2.and.k2.eq.i3.and.k3.eq.i1).or.
cilya     &                 (k1.eq.i3.and.k2.eq.i1.and.k3.eq.i2).or.
cilya     &                 (k1.eq.i3.and.k2.eq.i2.and.k3.eq.i1))go to 120
                 else if(m.eq.4)then
                 write(*,*)' neighbour matrix inconsistent'
                 write(*,*)' triangle',i,' has neighbour',k,' index',j
                 write(*,*)' triangle',k,' has no neighbour',i
