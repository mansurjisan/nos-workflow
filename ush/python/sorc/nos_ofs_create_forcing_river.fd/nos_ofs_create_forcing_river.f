...
C Input Data files:
C
C Output files: ${OFS}.river.$YYYY$MM$DD.t${HH}z.nc
C
...
C---------------------------------------------------------------------
      ...
      CHARACTER*200 START_TIME,END_TIME,NOSBUFR,USGSBUFR,PREFIXNOS      
...
cc Allocate arrays for RIVER 
      ...
      ALLOCATE(RTIME_F(NRIVERS,NTMAX))  
...
c  End of added by Zheng 
      ...
      ALLOCATE(river_f(NRIVERS,NTMAX)) 
...
!      IF(NWMUSED.EQ.1) THEN
      IF(NWMUSED.EQ.1.AND.MAXVAL(IUS).EQ.1) THEN
...
!      ELSEIF(trim(OCEAN_MODEL).EQ."SELFE") THEN
       ELSEIF(trim(OCEAN_MODEL).EQ."SELFE".or.trim(OCEAN_MODEL).EQ.
     &  "SCHISM") THEN
        ...
        ELSEIF(trim(OCEAN_MODEL).EQ."SCHISM") THEN
        OPEN(40,file='schism_flux.th',status='unknown')
        OPEN(41,file='schism_temp.th',status='unknown')
        OPEN(42,file='schism_salt.th',status='unknown')
        ENDIF


       WRITE(40,600) 0.0,(-river_transport(I,2),I=1,NIJ)  ! must start 0
       WRITE(41,600) 0.0,(tmp2d1(I,2),I=1,NIJ)
       WRITE(42,601) 0.0,(tmp2d2(I,2),I=1,NIJ)     
