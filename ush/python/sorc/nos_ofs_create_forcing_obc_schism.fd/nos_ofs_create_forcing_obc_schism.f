...
C----------------------------------------------------------------------------------
      ...
      character*200 FDIR,STYPE,VGRIDFILE,NCOM_FILE,T_NUDGE_FILE,PREFIXNOS
      character*200 NOSBUFR,USGSBUFR,VGRIDFILE_NU
...
!      GRIDFILE=TRIM(FIXofs)//'/'//TRIM(OFS)//'/'//trim(adjustL(BUFFER))
      ...
      if (trim(OCEAN_MODEL) .eq. "SCHISM" )then  
            read(5,'(a200)')BUFFER
         ...
         enddo
         VGRIDFILE_NU=trim(adjustL(BUFFER))
      endif  
...
!      NCOM_FILE=trim(adjustL(BUFFER))
      ...
      read(5,'(a200)')PREFIXNOS
      PREFIXNOS=trim(adjustL(PREFIXNOS))
...
!-----------------------------------------------------------------------
!      HC_FILE=TRIM(FIXofs)//'/nosofs.HC_NWLON.nc'
      HC_FILE='nosofs.HC_NWLON.nc'
      INQUIRE(FILE=trim(HC_FILE),EXIST=FEXIST)
      IF(.NOT. FEXIST)THEN
          WRITE(*,*)'Harmonic Constant NetCDF file is not found'
	  WRITE(*,*)'Please check nosofs.HC_NWLON.nc in FIX directory'
...
!     1    trim(OCEAN_MODEL) .EQ. "SELFE")THEN
      ...
      IF (trim(OCEAN_MODEL) .EQ. "FVCOM" .OR. trim(OCEAN_MODEL) .EQ. 
     1  "SELFE" .OR. trim(OCEAN_MODEL) .EQ. "SCHISM")THEN
...
!     1    trim(OCEAN_MODEL) .EQ. "selfe")THEN
       IF(trim(OCEAN_MODEL).EQ."SELFE".OR.trim(OCEAN_MODEL).EQ."selfe"
     1  .OR.trim(OCEAN_MODEL).EQ."SCHISM".OR.trim(OCEAN_MODEL).EQ.
     1  "schism")THEN

        ALLOCATE(weight_t_nudge(NODEm) )
        
        open(66,file="nudge_point_at_ofs_grid.dat")  !!! machuan
	...
	NUM_T_NUDGE=0
	  ...
	  if(weight_t_nudge(n) .GT. 0.0)then
	    NUM_T_NUDGE=NUM_T_NUDGE+1
            write(66,'(2I10)') NUM_T_NUDGE, N  !!! machuan
	  endif  
        enddo
...
!     1    trim(OCEAN_MODEL) .EQ. "selfe")THEN
      ...
      IF(trim(OCEAN_MODEL).EQ."SCHISM".OR.trim(OCEAN_MODEL).EQ."schism")
     &    THEN

!!!!!!!!!!!!!!  machuan work from here !!!!!!!!!!       

        write(*,*)'vgrid_nu=',trim(VGRIDFILE_NU)
        OPEN(40,file=trim(VGRIDFILE_NU))
        read(40,*) num_nu
        nvrt=kbm

        allocate( zcor_nu(num_nu,nvrt) )
        ...
        backspace(40)
           do kk=1,nb-1
             zcor_nu(kt,kk)=-1.0
!              zcor_nu(kt,kk)=-1.0-(nb-1-kk)*0.001  
           end do
        read(40,*) nn,nb, (zcor_nu(kt,kkk),kkk=nb,nvrt)
        end do    
         
         do kt=1,num_nu
           do kk=1,nvrt
            siglev(kt,kk)=zcor_nu(kt,kkk)
            enddo
          enddo
	 ...
	 TIME_clim(N)=day_end-day_start+(N-1)*DELT/86400.0
      ...
      DO N=1,NSTA
         ...
         IF(TS_FLAG(N) .GT. 0)THEN
            BUFFER1=trim(PREFIXNOS)//'.obc.clim.ts.'
...
!      IF (ALLOCATED(vbarOBC)) DEALLOCATE(vbarOBC)
        ...
        write(*,*) "== mmgp NOBC NTMAX== ",NOBC,NTMAX
...
c------------------------------------------------------------------
     ...
     *               NOBC,lonOBC,latOBC,ZOUT)
      ...
      ENDIF
        ...
        write(*,*) "=== mmgp I am not here ==="
        write(*,*) "=== DBASE_WL ===",DBASE_WL
        open(45,file="mmgp_wlobc.out")

        do n=1,ntmax
        write(45,*) TIMEOBC(N)
        write(45,*) (WLOBC(I,N),I=1,NOBC)
        enddo 
...
!        write(*,*) "====== mmgp NOBC RTOFS  ====", NOBC  !! 32613
...
!      write(*,*) "=== mmgp NOBC ===", NOBC  !!  number is 32613
...
!         DO K=1,KBm
!               zsigma(k)=-zcor_nu(i,k)*dp
!               oned1(k)= zsigma(K)
!          ENDDO

         DO K=1,KBm
            KR=KBm-K+1
            zsigma(KR)=-zcor_nu(i,k)*dp
!              KR=KBm-K+1
!               oned1(k)= zsigma(KR)
          ENDDO
...
!          write(*,*) "====== mmgp NOBC TS_NCOM_SELFE ====", NOBC       !!32613
...
!      write(*,*) "=== mmgp NTMAX NOBC ==", NTMAX, NOBC  19, 32613
!        stop   !  machuan
...
!           WL(I,J,ICOUNT)=WL(I,J,ICOUNT)+0.75 !! Model 0 is still 0.5 m bw xgeoid
           WL(I,J,ICOUNT)=WL(I,J,ICOUNT)+1.25
            ENDIF  
           ENDDO
           ENDDO   
           ENDDO   
...
!        write(33,35)ZETA_TIME(N),(WLOBC(I,N),I=1,NOBC,10),WLOBC(NOBC,N)
       ...
       write(33,* )ZETA_TIME(N),(WLOBC(I,N),I=1,NOBC)
...
!          WRITE(33,*)((TEMPOBC_M(I,K,N),I=1,NUM_T_NUDGE),K=1,KBm)


!        WRITE(33,*) TS_TIME(N),((TEMPOBC_M(I,K,N),K=1,KBm),I=1,NUM_T_NUDGE)
...
!      WRITE(33,*)((SALTOBC_M(I,K,N),I=1,NUM_T_NUDGE),K=1,KBm)
!      WRITE(33,*) TS_TIME(N),((SALTOBC_M(I,K,N),K=1,KBm),I=1,NUM_T_NUDGE)
...
C ----------------------------------------------------------------------------
        IF (trim(OCEAN_MODEL) .EQ. "SELFE") then !!!  machuan
...
!            ENDDO
	...
	smin=9999
        ...
        write(*,*) "=== mmgp final  NUM_T_NUDGE ==",NUM_T_NUDGE
...
! 	OPEN(82,file='elev3D.dat')
        ...
        open(80,file="bin_file_dimension_ctl")
        write(80,*) "ST time", "ST Kmb", "ST node", "WL time", "WL node", "DELT_TS", "DELT_WL"
...
!        OPEN(54,file='elev3D.th',form='unformatted')

        OPEN(37,file='temp_nu.in')
        OPEN(35,file='salt_nu.in')
        OPEN(54,file='elev3D.th')
...
!	  WRITE(35)ts_time(N)*86400.0
	  ...
	  ENDDO
	  ENDDO
!          WRITE(37,*)((tmp2d(I,K),I=1,NUM_T_NUDGE),K=1,KBm)
!          WRITE(37,*)((tempOBC_M(I,K,N),I=1,NUM_T_NUDGE),K=1,KBm)
          WRITE(37,*) time_seconds,((tempOBC_M(I,K,N),K=1,KBm),I=1,NUM_T_NUDGE)
...
!          WRITE(35,*)((tmp2d(I,K),I=1,NUM_T_NUDGE),K=1,KBm)
!          WRITE(35,*)((saltOBC_M(I,K,N),I=1,NUM_T_NUDGE),K=1,KBm)
          WRITE(35,*) time_seconds,((saltOBC_M(I,K,N),K=1,KBm),I=1,NUM_T_NUDGE)
...
555     FORMAT(100F10.4)
	IREC=0
        ...
        write(*,*) "=== mmgp NTMAX NODEM NOBC_ORI NTMAX_WL =="


        write(*,*) "=== mmgp final WL NOBC_ORI ===",NOBC_ORI
...
!            write(54,rec=IREC)time_seconds,(WLOBC(I,N),I=1,NOBC_ORI)
            write(54,*)time_seconds,(WLOBC(I,N),I=1,NOBC_ORI)

!	  endif  
!          WRITE(82,556)time_seconds,(WLOBC(I,N),I=1,NOBC_ORI)
          ENDDO

        write(80,888) NTMAX, NUM_T_NUDGE, KBm, NTMAX_WL, NOBC_ORI, delt_ts, delt
888     format(5I10, 2F10.1)
...
!        open(29,file="bin.structure.ctl")
!        write(29,*) NTMAX, NODEm, KBm
!        close(29)
