#!/bin/bash
# ============================================================================
# LEOFS Preprocessing Script
# Lake Erie Operational Forecast System - COMF/nosofs Framework
# Model: FVCOM (Finite Volume Community Ocean Model)
#
# This script runs LEOFS preprocessing using YAML configuration
# combined with the nosofs (COMF) shell scripts.
#
# Note: LEOFS is a Great Lakes model - no ocean boundary conditions or tides
#
# Usage:
#   ./run_leofs_prep.sh
#
...
echo "============================================"
echo "LEOFS Preprocessing"
echo "Lake Erie OFS (FVCOM)"
...
# LEOFS: No hindcast (cold start for lakes), 5-day forecast
export time_nowcastend=${PDY}${cyc}
export time_hotstart=${PDY}${cyc}    # Same as nowcastend for cold start
export time_forecastend=$(date -d "${PDY} ${cyc}:00 120 hours" +%Y%m%d%H)

echo "Date/Time configuration:"
echo "  PDY=${PDY}, cyc=${cyc}"
echo "  time_hotstart=${time_hotstart}"
echo "  time_nowcastend=${time_nowcastend}"
echo "  time_forecastend=${time_forecastend} (5-day forecast)"
echo ""
...
# You need leofs fix files (grid, forcing templates, etc.)
export FIXofs=${HOMEnos}/fix/leofs

# Working directory
export DATA=${HOMEnosofs}/work/leofs
...
export COMINhrrr=${DCOMROOT}/hrrr               # HRRR atmospheric (primary for Great Lakes)
...
# Note: No RTOFS for Great Lakes (no ocean BC)
...
export OFS_CONFIG=${HOMEnosofs}/nos_ofs/parm/systems/leofs.yaml
...
if [ $? -eq 0 ]; then
    ...
    echo "  OCEAN_MODEL=${OCEAN_MODEL:-FVCOM}"
    echo "  MINLON=$MINLON, MAXLON=$MAXLON"
    echo "  MINLAT=$MINLAT, MAXLAT=$MAXLAT"
    echo "  LEN_NOWCAST=${LEN_NOWCAST:-0} hours (cold start)"
    echo "  LEN_FORECAST=${LEN_FORECAST:-120} hours"
    echo ""
else
...
# Set FVCOM-specific defaults if not set
export OCEAN_MODEL=${OCEAN_MODEL:-FVCOM}
...
if [ ! -d "${FIXofs}" ]; then
    ...
    echo ""
    echo "LEOFS requires fix files including:"
    echo "  - leofs.ctl (control file)"
    echo "  - leofs_grd.dat (FVCOM grid file)"
    echo "  - leofs_dep.dat (depth file)"
    echo "  - leofs_obc.dat (open boundary - Detroit River connection)"
    echo "  - leofs_spg.dat (sponge layer)"
    echo "  - leofs_cor.dat (Coriolis)"
    echo ""
...
# PREPROCESSING STEPS
# ============================================================================
echo "=== LEOFS Preprocessing Steps (FVCOM - Great Lakes) ==="
echo ""
echo "Note: LEOFS is a Great Lakes model with special characteristics:"
echo "  - No ocean boundary conditions (lake is enclosed)"
echo "  - No tidal forcing (Great Lakes have negligible tides)"
echo "  - HRRR is primary atmospheric source (high resolution for lakes)"
echo "  - Detroit River inflow from Lake Huron"
echo ""
echo "The COMF framework runs these steps via exnos_ofs_prep.sh:"
echo "  1. nos_ofs_launch.sh - Set up OFS configuration"
echo "  2. nos_ofs_create_forcing_met.sh - HRRR/GFS atmospheric forcing"
echo "  3. nos_ofs_create_forcing_river.sh - River forcing (Detroit, Maumee, Cuyahoga)"
echo "  4. nos_ofs_prep_fvcom_ctl.sh - FVCOM control files"
echo ""
echo "Skipped for Great Lakes:"
echo "  - nos_ofs_create_forcing_obc.sh (no ocean BC)"
echo "  - Tidal forcing (negligible tides)"
echo ""

# Check if input data exists
if [ ! -d "${COMINhrrr}" ] && [ ! -d "${COMINgfs}" ]; then
    echo "WARNING: Input data directories not found!"
    echo "  COMINhrrr=${COMINhrrr}"
    echo "  COMINgfs=${COMINgfs}"
    echo ""
    echo "To run the full preprocessing:"
    echo "  1. Provide input data (HRRR/GFS, NWM)"
...
echo "Region: Great Lakes - Lake Erie"
...
echo "  Atmospheric: HRRR (primary), GFS (fallback)"
echo "  Rivers: NWM + USGS (Detroit, Maumee, Cuyahoga)"
echo "  Ocean BC: None (enclosed lake)"
echo "  Tides: None (Great Lakes)"
echo ""
