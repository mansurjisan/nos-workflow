...
Still required before a coupled run, and NOT in this archive:
  - <prefix>.param.nml         : shipped in the repo at fix/stofs_3d_ak_ufs/
  - <prefix>.bctides.in_template : generated by prep (validate against
                                   r09a_reference/bctides.in, --reference)
  - hotstart / restart          : R09a's hotstart0.nc is 12 GB and is keyed to
                                  its own 2019-07-01 12z start; a cold start or
                                  a prep-built restart is expected instead.
  - partition.prop is staged above and targets 2393 compute ranks, matching
    ufs_coastal.schism_tasks. Regenerate with tools/gen_stofs_partition_prop.sh
    if that rank count ever changes.
EOS
