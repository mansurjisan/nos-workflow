...
#################################################################################################################
#  Name: exstofs_3d_pac_watchdog.sh                                                                             #
...
# ----------------------->
  fn_this_script=exstofs_3d_pac_watchdog.sh
...
# -----------------------> check for available hotstart_x.nc
  ...
  ### dt_timestep=150 atlantic
  # dt_timestep=90 pacific RUN31K-L

# -----------------------> COMOUT dir  
    dir_watchdog_COMOUT=${COMOUT}/outputs_watchdog
    echo "exstofs_3d_pac_watchdog.sh: dir_watchdog_COMOUT=${dir_watchdog_COMOUT}"
...
if [ 1 -eq 1 ]; then
   ...
   dir_nco_tmp_archive=${TMPPATH}_$pid_jobs_NowForeCast
       echo "exstofs_3d_pac_watchdog.sh: dir_nco_tmp_archive=${dir_nco_tmp_archive}"    
  ...
  # ----------> check now/forecast progress; merge global hotstart; archive global history NC
   ...
   fn_mirror=${dir_NF_run_outputs}/mirror.out
     ...
     str_egrep_TIME_STEP='(\s480;|\s960;|\s1440;|\s1920;|\s2400;|\s2880;)'
   ...
   for i_no in ${rem_list_cnt_no[@]}; do
     ...
     while [[ 1 ]]; do
	  ...
	  cpreq  -fp  ${dir_NF_run_outputs}/local_to_global_0*  ${dir_nco_tmp_archive}
         ...
         export err=$?
         pgm=${SCRstofs3d}/exstofs_3d_pac_watchdog.sh
	    ...
	    echo
            ...
            if [[ ${i_time_step} -eq ${TIME_STEP_end_of_FCAST} ]]; then
                 ...
                 # This is handled in exstofs_3d_pac_now_forecast.sh
		 ...
		 msg=" Model is running for the ${TIME_STEP_end_of_FCAST}; its reults are to be copied to COMOUT by now/forecast script after model run" 
                 msg="${msg}\n In exstofs_3d_pac_watchdog.sh: exstofs_3d_pac_now_forecast.sh completed. Now to quit watchdog job.\n"
		 echo -e ${msg}
