...
##############################################################################
#  Name: exstofs_3d_pac_post_3.sh                                            #
#  This script is a postprocessor to create the hotstart file, namely        #
#  stofs_3d_pac.t12z.hotstart.stofs3d.nc and copies the files to the com     #
#  directory. If the current model run date is April 5, the T/S fields in #
#  hotstart file would be a combined STOFS-3D-pac outpus and G-RTOFS results.#
#                                                                            #
#  Remarks:                                                                  #
#                                                        September, 2022     #
##############################################################################
...
# ----------------------->
  ...
  fn_this_script=exstofs_3d_pac_post_3
  ...
  # MMDD_ANNUAL_UPDATE_RESTART_FILE=0405
   MMDD_ANNUAL_UPDATE_RESTART_FILE=0528
   MMDD_ANNUAL_UPDATE_RESTART_FILE=0602
   MMDD_ANNUAL_UPDATE_RESTART_FILE=0821
   MMDD_ANNUAL_UPDATE_RESTART_FILE=0829
   MMDD_ANNUAL_UPDATE_RESTART_FILE=0416
   MMDD_ANNUAL_UPDATE_RESTART_FILE=1117
   MMDD_ANNUAL_UPDATE_RESTART_FILE=1122
  ...
  if [[ ${MMDD_FCAST_BEGIN} == ${MMDD_ANNUAL_UPDATE_RESTART_FILE} ]]; then
    ...
    echo
      ...
      export pgm="${USHstofs3d}/stofs_3d_pac_create_restart_combine_rtofs_stofs.sh"
      ${USHstofs3d}/stofs_3d_pac_create_restart_combine_rtofs_stofs.sh >> ${file_log} 2>&1
