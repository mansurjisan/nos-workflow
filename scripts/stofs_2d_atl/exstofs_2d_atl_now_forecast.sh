...
##############################################################################
#  Name: exstofs_2d_atl_now_forecast.sh                                      #
#  This script conducts the barotropic (2D) nowcast and forecast             #
#  simulations using the unified 4-step framework:                           #
...
#                                                                            #
#  Identical structure to exstofs_3d_atl_now_forecast.sh but with            #
#  BAROTROPIC=true, which causes the shared model_run library to skip        #
#  T/S OBC and nudge file staging.                                           #
#                                                                            #
#  Remarks:                                                                  #
#    February 2026 — barotropic 2D ATL variant                               #
##############################################################################
...
# -----------------------> Initialize
  ...
  fn_this_script="exstofs_2d_atl_now_forecast.sh"

# Fallback if postmsg not provided by prod_util module
  command -v postmsg >/dev/null 2>&1 || postmsg() { echo "[postmsg] $*"; }

  msg="${fn_this_script} started (unified 2-phase barotropic nowcast/forecast)"
  echo "$msg"
  postmsg  "$msg"
  ...
  # Barotropic mode flag — used by shared library to skip T/S files
...
if [ "${RUN_FORECAST:-YES}" = "YES" ] && [ ${LEN_FORECAST:-108} -gt 0 ]; then
  ...
  echo "========================================="
  echo "=== FORECAST PHASE (barotropic) ==="
  echo "========================================="
